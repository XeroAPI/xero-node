...
const { forEach, merge, clone, isFunction, isEmpty, isUndefined, defaults, isArray } = require('lodash');
...
function Application(options) {
  this.options = merge(clone(Application.defaults), options);
...
Object.assign(Application.prototype, {
  init: function() {
        ...
        return new Promise(function(resolve, reject) {
            ...
            function getResource(offset) {
                ...
                if (offset) {
                    ...
                    if (options.other) {
                        forEach(options.other, function(value, key) {
                ...
                self.oa.get(url, self.options.accessToken, self.options.accessSecret, function(err, data, res) {
                  ...
                  if (options.pager && options.pager.callback) {
                      options.pager.callback(err, ret, function(err, result) {
                          result = defaults({}, result, { recordCount: 0, stop: false });
                          if (!result.stop)
    ...
    convertEntities: function(obj, options) {
        ...
        if (isArray(entitiesTop)) {
            entitiesTop.forEach(function(entityObj) {
                addEntity(entityObj);
            })
        } else {
