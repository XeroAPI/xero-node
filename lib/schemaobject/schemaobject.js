const { forEach, isUndefined, isFunction, isObject, isArray, isBoolean, isString, toArray, isDate, isNumber, size, clone, difference, includes, defaults, isNull, isEmpty, matches } = require('lodash');
...
var normalizeProperties = function(properties) {
    ...
    if (isObject(properties.type) && !isUndefined(properties.type.type)) {
        forEach(properties.type, function(value, key) {
...
var SchemaObject = function(schema) {
    return function(defaultValues) {
        ...
        forEach(schema, function(properties, index) {
            if (!properties) {
        ...
        this.validate = function(options, parentPath) {
            ...
            function validateItem(index, value, properties) {
                forEach(properties.validators, function(settings, validatorType) {
...
Object.assign(Tracking.prototype, {
    clear: function() {
        var self = this;
        forEach(this._instance._schema, function(properties, index) {
    ...
    hasChanged: function(index) {
        return !!this._changes.find(matches({ index: index }));
    }
})
